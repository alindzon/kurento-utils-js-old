...
function WebRtcPeer(mode, options, callback) {
  ...
  /**
  * Aug 2022 added by efficient apps to add audioSource and videoSource option for setSinkId
 *  not working 
  ...
  this.processAnswer = function (sdpAnswer, callback) {
    ...
    logger.debug('SDP answer received, setting remote description:',pc.signalingState)
 ...
 * this test failed, this does not work
 
    logger.debug('audioSource',audioSource);
    if (audioSource && localVideo) localVideo.setSinkId(audioSource);
    logger.debug('videoSource',videoSource);
    if (videoSource && localVideo) localVideo.setSinkId(videoSource);
...
Object.defineProperties(WebRtcPeer.prototype, {
  'enabled': {
    ...
    set: function (value) {
      logger.debug('line 827 enabled does this work or cause an error?')
      this.audioEnabled = this.videoEnabled = value
      logger.debug('line 829 afterenabled')
    }
  },
...
WebRtcPeer.prototype.dispose = function () {
  ...
  try {
    ...
    // july 2022 causing errors  pc.getLocalStreams().forEach(streamStop)

      // FIXME This is not yet implemented in firefox (tested variable not found)
